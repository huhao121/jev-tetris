...
// On Vercel the deployment's OIDC token arrives per request: in the request
// context the platform attaches to globalThis, and as the x-vercel-oidc-token
// header. Locally, AI_GATEWAY_API_KEY or VERCEL_OIDC_TOKEN in the environment.
function contextOidcToken(req) {
  try {
    const ctx = globalThis[Symbol.for("@vercel/request-context")]?.get?.();
    const fromContext = ctx?.headers?.["x-vercel-oidc-token"];
    if (fromContext) return fromContext;
  } catch {
    /* no request context */
  }
  return req?.headers?.["x-vercel-oidc-token"] || null;
}

export function gatewayCredential(req) {
  if (process.env.AI_GATEWAY_API_KEY) return { token: process.env.AI_GATEWAY_API_KEY, method: "api-key" };
  const oidc = contextOidcToken(req) || process.env.VERCEL_OIDC_TOKEN;
  if (oidc) return { token: oidc, method: "oidc" };
  return null;
}

export function hostedJevAvailable(req) {
  return gatewayCredential(req) !== null;
}
...
export async function forwardToGateway({ body, req }) {
  const cred = gatewayCredential(req);
  if (!cred) {
    return jsonResult(503, {
      detail: { error_type: "not_configured", message: "Hosted Jev is not configured on this server (no AI_GATEWAY_API_KEY and no Vercel OIDC token). Enter a TypeSafe key instead." },
    });
  }
  let request;
  try {
